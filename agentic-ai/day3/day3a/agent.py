...
# Key differences for DatabaseSessionService:
...
async def run_session_go():
    ...
    # Comment out the first query to demonstrate that there is no memory between sessions
    await run_session(
        runner,
        [
...
# asyncio.run(run_session_go())


# Testing with a different session to verify session data is isolated
# async def run_session_go2():
#     # Run a conversation with two queries in the same session
#     # Notice: Both queries are part of the SAME session, so context is maintained
#     # Comment out the first query to demonstrate that there is no memory between sessions
#     await run_session(
#         runner,
#         [
# #            "Hi, I am Alex! What is the capital of France?",
#             "Hello! What is my name?",  # This time, the agent should remember!
#         ],
#         "stateful-agentic-session-2",
#     )

# asyncio.run(run_session_go2())

# how are events stored in the database?
import sqlite3
...
# Context Compaction
# All the events are stored in full in the session Database, and this quickly adds up. 
# Let's use ADK's Context Compaction feature to see how to automatically reduce the context stored in the Session.


# Re-define the same chatbot_agent app with Events Compaction enabled
...
async def run_session_compact():
    ...
    ## Verifying compaction in the session history -- everything changed was behind the scenes!
...
# asyncio.run(run_session_compact())



## Working with Session State
# creating custom tools to manually capture and save characteristics about the user in the session state.
