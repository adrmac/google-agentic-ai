...
async def run_currency_agent_test():
...
#     asyncio.run(run_currency_agent_test())


# Improving agent reliability with code execution
##### 💡 Let's ask our agent to generate a Python code to do the math, and run it to give us the final result! Code execution is much more reliable than having the LLM try to do math in its head!

## Built-in Code Executor Tool
# ADK has built-in code execution capabilities that can be easily added to any LLM agent.
# This uses Gemini's code execution environment under the hood.
...
# update currency_agent instructions to generate code, not final amount
# add the calculation_agent as a tool to currency_agent
...
async def run_enhanced_currency_agent_test():
...
if __name__ == "__main__":
    asyncio.run(run_enhanced_currency_agent_test())